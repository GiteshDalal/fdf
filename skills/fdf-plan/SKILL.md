---
name: fdf-plan
description: Use when an FDF feature is specified (SPEC.md approved) and has no implementation plan yet — before touching code.
---
...
Plan within the documented project context: `STACK.md`, `ARCHITECTURE.md`, and
`INFRA.md` at the bundle root tell you the real technologies, code
organization, and infrastructure — task `resource:` paths, tools, and test
commands must match them, not invented ones.

**Write for a zero-context implementer.** Whoever executes a task may be a
fresh agent that sees only that task file and SPEC.md — no session memory,
no neighboring tasks. Every name, signature, path, endpoint, and payload
shape a task needs must be written in the task itself. "As discussed" and
"similar to task 01" are plan failures.
...
1. Read the feature doc and `<group>/<slug>/SPEC.md`; run `fdf validate`.
   **Survey the project before writing anything**: find the real
   ...
   paths must exist (R1); invented paths are confidently wrong.
2. **SPEC gap → ask.** If a task needs a contract SPEC.md doesn't define
   (a column schema, an API shape, a naming rule), STOP and ask the user;
   record the answer in SPEC.md — their answer is the approval for that
   edit. Never invent it mid-plan.
...
4. **Decompose** into tasks `01-slug.md`, `02-slug.md`, … (`type: Task`,
...
5. **Write TEST.md** (`type: Test`): `# Test Cases`, one entry per scenario,
   naming the scenario verbatim and giving the CONCRETE verification — the
   exact command, the test file/name to write, or a step-by-step manual
   procedure — **and what passing looks like** (expected status, output,
   resulting state). "Run the tests" proves nothing. If verification isn't
   obvious, STOP and ask how done-ness will be proven; record the answer.
   New APIs get an E2E/integration test; UI changes get a real-browser
   (Playwright) check.
6. **The final task always satisfies TEST.md** — writing/running what it
   names. Every plan ends with it; it depends-on every other task.
7. **Write PLAN.md** (`type: Plan`): `# Tasks` — ordered list linking every
   task file. Plan order is the readable order; depends-on is execution truth.
8. Flip feature status to `planned`; LOG.md entry; `fdf validate` exit 0
