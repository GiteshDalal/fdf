...
- `# Terms` — **required** and parsed. One `## <Term>` heading per canonical
  ...
  - `- instead-of: <word>[, <word>…]` — optional: the words this term
    ...
    Item" over a banned "item") is not a match.
...
found in a feature's Gherkin (F12). Prose is deliberately not checked: natural
language reuses these words for unrelated purposes, and a check that cries
wolf is a check that gets switched off. The scan is scoped to Feature
documents and to change and fix declarations for the same reason — it keeps
the vendored `SPEC.md`, whose examples are somebody else's vocabulary, out of
the match set.
...
un-landed regression case); a banned domain word in Gherkin (F12, an error
under `--strict-domain`); a filled `DOMAIN.md` with no terms; an `active`
practice with no `applies-to`, which no work can be routed to by path; an
`open` or `accepted` debt with no `resource`, which nothing can route to
either.
