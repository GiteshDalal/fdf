# Feature Document Format (FDF) — v0.6
...
rumour. Work that arrives **after a feature is delivered** — a change request
(`type: Change`) or a defect (`type: Fix`) — lives under `changes/`
...
| | Documents | On change |
|---|---|---|
| **Living** — describes the system as it is **today** | `Feature` (Gherkin), `Test`, `Surface`, `Practice`, `Debt`, the five `Context` documents | **Amended in place.** A living document that describes behavior the software no longer has makes the bundle lie. |
| **Episodic** — a frozen record of **one piece of work** | `Spec`, `Plan`, `Task`, `Change`, `Fix`, `Log` entries | **Never rewritten**, except by a lexicon fix (below). New work gets a new episode; the old one stands as the record of why things were done the way they were. |
...
**lexicon fix**, which brings the wording into line with `DOMAIN.md` and
changes nothing else — a banned word replaced by the term it is banned in
favour of, or a Gherkin step that quoted a surface's label reworded to name
the concept, the label moving to the step definition or the test. It changes
no fact about the work and nothing that was delivered, so it needs no
`Change` or `Fix`, and a bundle that bans a word is swept, frozen records
included, until it speaks one vocabulary. A
scenario name is fixed everywhere it appears in the same edit — the Gherkin,
`slug.test.md`, task acceptance, and every change or fix that names it —
because names are the joins F8 and F10 check. Quoted surface wording, which
the lexicon does not govern, and document IDs stay as they are: renaming a
document is a move, not a word fix.
...
do `changes/`, `practices/` and `debts/` and every group inside them. Trail
...
`[a-z0-9-]`, practice and debt slugs and their group names `[a-z0-9-]`, tasks
`NN-[a-z0-9-]+.md`, releases `<version>.md`. `changes/`, `practices/`,
`debts/` and `releases/` are reserved lowercase directory names at the bundle
root and are not themselves groups. Any other uppercase filename is an error.
...
`Change`/`Fix` file outside `changes/`, a `Practice` outside `practices/`, or
a `Debt` outside `debts/`.
...
filed in a group. Practice and debt IDs follow the same rule:
`practices/<slug>`, `debts/<group>/<slug>`, and so on.
...
`fdf_version: "0.6"` plus a link to this document.
...
recorded in `debts/LOG.md` (`fdf debt --cleanup` does both in one step). This
is the one place FDF removes a document, and it is allowed for exactly one
reason: unlike a `retired` feature or a `superseded` practice, a resolved debt
describes nothing that still exists. What must survive is the fact that the
debt was paid and what paid it — which is why it moves to the log rather than
simply being deleted. An `open` or `accepted` debt is never removed.
...
one valid `.feature` file — that concatenation is the test for whether the
split across headings is right.
...
- `# Terms` — **required** and parsed. One `## <Term>` heading per canonical
  ...
  - `- instead-of: <word>[, <word>…]` — optional: the words this term
    ...
    *Living and episodic documents*), finished work included.
...
Validators check the lexicon for internal consistency and report banned words
found in a feature's Gherkin and in the scenario names a change or fix
declares (F12). Prose is deliberately not checked: natural language reuses
these words for unrelated purposes, and a check that cries wolf is a check
that gets switched off. The scan is scoped to Feature documents and to those
declared names for the same reason — it keeps the vendored `SPEC.md`, whose
examples are somebody else's vocabulary, out of the match set.
...
A lexicon fix is neither: it changes the Gherkin's words, not what the
software does, and is made in place (see *Living and episodic documents*).
...
bundle. Cross-linking a feature with the documentation of the code that
...
## 2026-01-05
* **Initialization**: scaffolded by `fdf init` (FDF v0.6).
```
...
description: The FDF v0.6 specification this bundle conforms to.
...
| F12 | **Domain integrity.** `DOMAIN.md` carries a `# Terms` section; every `## <Term>` heading is unique and followed by a non-empty definition; no canonical term appears in another term's `instead-of`; no `instead-of` word is claimed by two terms. A banned word matched whole-word, case-insensitively and tolerating a regular plural, inside the Gherkin fence of a **Feature document**, or in a scenario name a `Change` or `Fix` declares — an `add:`, `modify:` or `remove:` entry, or a regression case's scenario name — at any status, is a **warning**, promoted to an error by strict domain mode (`fdf validate --strict-domain`). A `## <feature-id>` heading, a regression case's verification, and prose in the section are not scanned |
...
| R1 | Every `Task`, `Change`, `Fix` and `Debt` `resource` path and every `Practice` `applies-to` path exists in the project (skipped with a warning for standalone bundles) |
...
un-landed regression case); a banned domain word in Gherkin or in a declared
scenario name (F12, an error under `--strict-domain`); a filled `DOMAIN.md`
with no terms; an `active` practice with no `applies-to`, which no work can be
routed to by path; an `open` or `accepted` debt with no `resource`, which
nothing can route to either.

`--strict-domain` is off by default and deliberately so. The lexicon check is
the one rule in FDF that matches natural language rather than structure, and
false positives are certain in a young bundle — the words a term replaces are
ordinary English before they are banned. Warnings let a project adopt the
lexicon incrementally; a project that has finished that adoption turns the
flag on in CI and keeps it clean from then on.
...
Every F12 finding can be cleared honestly, because the lexicon fix reaches
every document it scans: finished changes, fixes and retired features take
it like any other. F12 checks the words the bundle chose — every feature's
Gherkin and every scenario name a change or fix declares — and leaves out
text that quotes what the lexicon does not govern: a `## <feature-id>` heading
quotes a document ID, and a regression case's verification quotes a command,
a path, or a surface's wording, exactly as the `slug.test.md` case it mirrors,
which F12 does not scan either. A rename recorded before its words were fixed
...
The `Practice` and `Debt` types, `practices/`, `debts/`, `DOMAIN.md`, F11, F12
and F13 apply to bundles pinning v0.6. F10, the `Change` and `Fix` types, the
`retired` feature status, feature `depends-on`, and releases listing changes
apply to bundles pinning v0.5 and later. Bundles pinning earlier versions are
validated under their pin: v0.5 has `changes/` and four Context documents, but
...
migration between adjacent versions (`fdf migrate`). This document is v0.6;
`fdf migrate` upgrades a v0.5 bundle by rewriting the pin, re-vendoring the
bundle's `SPEC.md`, scaffolding `practices/INDEX.md` and `debts/INDEX.md`, and
scaffolding a `DOMAIN.md` stub. No existing document moves, changes shape, or
needs editing, and a v0.5 bundle that never writes a practice or a debt
validates identically under
v0.6 — with one exception, which is the point of the version: `DOMAIN.md` is a
fifth Context document, so F9 holds the bundle to filling it before the next
feature. The migration itself never fails on that (fresh stubs are advisory
during `fdf migrate`); the `fdf-init` interview fills the lexicon, and until it
does, feature work is gated exactly as it is for the other four.
...
7. `fdf validate` before committing.
...
- **2026-09-23 — lexicon fixes.** As released, F12 reported a banned word in
  every `Change` and `Fix` declaration, finished ones included, while
  episodic documents could never be rewritten, and F10 required a `remove:`
  entry to quote its name verbatim. Under `--strict-domain` a bundle holding
  a banned word in finished work could not pass honestly. F12 also scanned
  the whole declaration section, so a feature ID or a quoted surface string
  there was reported with no correct fix. Three corrections: a lexicon fix —
  replacing a banned word with its term — is allowed in every document,
  episodic ones included; F12 scans the declared scenario names (`add:`,
  `modify:`, `remove:`, regression cases), not the section's headings,
  verifications or prose; and F10 reads a name one `Change` both removes and
  adds as a replacement, so a rename survives the fix of its words.
