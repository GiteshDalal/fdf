...
when git would ignore a file where migrate puts it, one git tracks that
moves or one migrate writes, which git could then neither show nor undo (a
directory git ignores as a whole is named once); and when the pin is newer
than it knows, or a 0.x version FDF never had. A bundle already pinned to
1.0 is repaired in place, and nothing moves: a missing index or Context
stub is restored, and so is `SPEC.md` when it is missing or not 1.0's own
text.
