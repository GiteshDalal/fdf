...
would be the `INDEX.md` or `LOG.md` beside it. The register names `features`,
`changes`, `practices`, `debts`, `bugs` and `releases` are reserved at the
bundle root, and only there: a register is not a group, and inside one they
are ordinary names.
...
- (c) A directory beside a document of the same name belongs to it: beside a
  ...
  practice, debt or bug it is an error when it holds Markdown. Every other
  directory in a register that holds Markdown is a group.
...
9. **One entry** in the root `LOG.md` records the migration, with counts.
   ...
   `fdf install`, review the diff, and commit. In a git repository they
   also give the commands that back the migration out: `git reset` of what
   migrate marked with `git add -N`, on which `git stash` and `git clean`
   would trip, then the commands that put everything back after a
   migration that stops partway.

Migrate refuses, before it writes anything, when the bundle, its root
`INDEX.md`, `LOG.md` or `SPEC.md`, a register it writes into, whatever that
holds, or a directory of Markdown in it is a symbolic link; when a Markdown
file at the root has no place in 1.0; when a document is named `index.md`
or `log.md`; when one of 0.1's renames, or 0.3's trail lift, would land on
a file that is there, as a disk that reads case can hold `index.md` and
...
would write has uncommitted changes, or the bundle holds files git does not
track, ignored ones included, but for a hidden file git ignores; when git
