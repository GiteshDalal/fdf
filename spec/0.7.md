...
| `timestamp` | Recommended. When the document last changed in substance: a date (`2026-02-14`) or an RFC 3339 time (`2026-02-14T09:30:00Z`); anything else is an error (F1). Both are read in UTC: a date is a UTC date, and a time written with an offset (`+10:00`) is the moment it names, which falls on a UTC date. The fdf commands write UTC |
...
`depends-on`. When one of those scenarios would stop being true, the
...
`debts/LOG.md`, and its file and its listing in the index beside it are
removed (`fdf debt --cleanup` does all three). The
...
- (b) A `timestamp`, when it has a value, is a date (`2026-02-14`) or an RFC
  3339 time: `2026-02-14T09:30:00`, optionally with a fraction of a second,
  then `Z` or an offset such as `+10:00`. A time with neither is an error,
  because F10 cannot tell which moment it names. An empty `timestamp` counts
  as a missing one, which is a warning (see *Soft checks*).
...
- A `timestamp` is a date or an RFC 3339 time with `Z` or an offset (F1). A
  ...
  or offset it was written in, or keep only its date.
