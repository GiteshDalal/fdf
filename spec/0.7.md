...
|---|---|---|---|
| F1 | Frontmatter, log headings, the version pin | F9 | Context documents filled in |
...
| **register**, **cleared** | The debts under `debts/`, or the bugs under `bugs/`. A `resolved` entry is cleared by recording it in the register's `LOG.md` and removing its file. |
...
| `timestamp` | Recommended. When the document last changed in substance: a date (`2026-02-14`) or an RFC 3339 time (`2026-02-14T09:30:00Z`) |
...
As a / I want / So that, is a **new feature** rather than a Change. The new
feature may record where it came from in `depends-on`.
...
`debts/LOG.md` and its file is removed (`fdf debt --cleanup` does both). The
two registers, debts and bugs, are the only places FDF removes a document,
and only for this reason: unlike a `retired` feature or a `superseded`
practice, a resolved entry describes nothing that still exists. The log line
keeps the fact that the entry was closed and what closed it. An `open` or
...
places: as a case in `slug.test.md` (F8), and in the `# Acceptance` of the
task that proves it (not checked). These repeated names are **joins**, matched
...
2. Before a Change modifies or removes a capability's scenarios, backfill
   those scenarios. A Change that only adds scenarios needs no backfill.
...
- `# Violates`: optional. For each feature with a scenario the defect
  ...
  `affects` (F14); under it, one `- <scenario name>` line per contradicted
...
- **`slug.plan.md`** (`type: Plan`): a `# Tasks` section with an ordered list
  ...
  `slug.test.md`.
- **`slug.test.md`** (`type: Test`): `# Test Cases`, with one case per
  scenario, giving the scenario's name word for word and its concrete
  verification: a command, a test file path, or an explicit manual
  procedure. Each case SHOULD be a `## <scenario name>` heading followed by
  its verification. `# Setup` is optional. From `planned` on, the file
  contains every scenario name (F8). Living: it is the feature's standing
  regression suite, and every Change or Fix that changes or proves the
  feature's scenarios amends it.
...
needs no surface document. For a built feature it is written once the design is
approved, beside the spec. For an adopted feature it records the interfaces
the code already has, read from the code.
...
entry at the top of its date. Its entries are episodic: an entry is never
...
- `# Regression cases`: **required** (F10). Exactly one `## <feature-id>`
  ...
  list item per scenario the Fix proves, on one line:
...
warning otherwise), and neither has a `LOG.md`. Every `INDEX.md` has a
bulleted list of links (a warning otherwise). None of them is a document,
which is why none has a `type`.
...
that the file contains every scenario name, which is why renaming a scenario
is never a one-file edit. The test document is living: the Fix of 2026-02-26
...
because someone would have had to decide. Each regression case is one line,
so its verification sits after the dash on the same line.
...
**F1. Frontmatter, log headings and the pin.**
...
- (b) Every `##` heading in a log, whether a `LOG.md` or a `slug.log.md`, is
  an ISO 8601 date: `## YYYY-MM-DD`.
- (c) The root `INDEX.md`'s `fdf_version`, when present, is a version the
...
and has no `version`; and a `retired` feature with a plan or a task directory
keeps its spec.
...
first scenario, `slug.test.md` exists (`type: Test`) and contains the name of
every scenario. A retired feature is exempt.
...
- (c) Every declaration entry is a `-` or `*` list item under a feature's
  heading: on a Change, an `add:`, `modify:` or `remove:` line; on a Fix, a
  `<scenario> — <verification>` line. Each feature has one heading.
...
- (g) Every regression case names a scenario that exists in its feature,
  gives a verification, and has a case in that feature's `slug.test.md`.
...
the feature. *Later* means a later `timestamp`; when the order cannot be
told, because a timestamp is missing or both give only the same date, the
check applies (see *Notes* below). On features, F10 also checks:
...
- (b) Each `affects` ID names an existing feature. Every `# Violates` entry
  is a `- <scenario name>` line, optionally followed by ` — <note>`, under the
...
pin, re-vendoring the bundle's `SPEC.md` and scaffolding `bugs/INDEX.md`; no
...
- Every entry under `# Scenario changes` is an `add:`, `modify:` or
  `remove:` line under a feature's heading (F10).
...
- The exceptions for removed and re-added scenario names are ordered by
  `timestamp`: only a later document excuses an earlier one (F10).
