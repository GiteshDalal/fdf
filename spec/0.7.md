...
- A **lexicon fix** brings the wording into line with `DOMAIN.md`: a banned
  ...
  the step definition, `slug.test.md` or `slug.surface.md`. The mapping is the
  lexicon, and a bundle
...
- A **feature** `<group>/<slug>.md` owns **stem-qualified trail siblings** at
  ...
  and its only possible siblings are `slug.test.md`, `slug.surface.md` and
...
No status requires `slug.surface.md` or `slug.log.md`. A `draft` feature has
neither, because it has no trail siblings at all; from `specified` on, either
may exist (see *Surface documents* and *Logs* for when to write them).
`retired` is terminal and reversible only by a new
...
text is not kept here: the Change that altered it names each scenario it
added, modified or removed, and version control keeps the old wording. A
feature document carries no
...
change in the bundle-root `LOG.md` (see *Logs*). Until the interview has
...
`practices/<slug>.log.md`. The ceremony is lighter than
...
- **`slug.surface.md`** (`type: Surface`) — every interface the feature
  ...
  Living. Written whenever the feature has a surface; no status requires it.
  See *Surface documents* below.
- **`slug.log.md`** (`type: Log`) — what happened to the feature and why,
  newest first. Created by its first entry; no status requires it. See
  *Logs* below.
...
job whose output nobody reads directly or a change to how data is stored, has
no surface document. For a built feature it is written once the design is
...
rewritten once written: a later entry corrects an earlier one. The one
exception is a maintenance edit, which repairs an entry's links and leaves its
words alone.

Three kinds of file are logs. The bundle-root `LOG.md` and a group's `LOG.md`
are reserved files with no frontmatter. A `<slug>.log.md` beside a feature,
change, fix, practice, debt or bug is a document of `type: Log`. The registers'
own logs, `debts/LOG.md` and `bugs/LOG.md`, hold one line per cleared entry,
...
| The bundle: initialization, migration, a Context document edit, a move, a lexicon fix, an adoption batch, a release, a checkpoint | the bundle-root `LOG.md` |
...
**Creating one.** A log is created by its first entry. `fdf log <id>
"<entry>"` writes the entry in the right log and creates the log when it does
not exist yet; with no ID it writes to the bundle-root `LOG.md`. A `draft`
feature has no log, because it has no trail siblings (F4): its first entry
comes with its approved spec.
...
## 2026-01-12
* **Adopted**: 3 capabilities mapped in payments/, card-payments among them.
...
is in that feature's log (see *Log* below). The banned word is written in a
...
2. Advance via stem trail files: `slug.spec.md` (→ `specified`), then
   ...
   (→ `implementing`, → `done`). When the feature has a surface, write
   `slug.surface.md` beside the spec (*Surface documents*). Keep `# Tasks` in
   lockstep; the final task satisfies `slug.test.md`.
3. Link from the group `INDEX.md`. Log each step in the log of the document
   it concerns: `fdf log <id> "<entry>"` (*Logs*).
