...
2. Before a Change alters behavior the capability already has, backfill
   ...
   that adds behavior the code does not have at all needs no backfill. What
   decides is the code, not the scenario list: a map entry has no scenarios,
   yet a Change can still alter what it does.
...
- (c) Every declaration entry is a `-` or `*` list item under a feature's
  ...
  on a Fix, `<scenario> — <verification>`. An entry is its item's whole
  text: a continuation line, indented or not, is part of it, so a long entry
  may wrap, and a blank line ends it. Each feature has one heading.
